...
export async function actualizarJuego(req,res,next) {
    try{
        ...
        if(Object.keys(datos).length==0){
            return res.status(400).json({error:"Debes evitar al menos un campo válido para actualizar "})
        }
        const juego= await juegoService.actualizar(req.params.id,datos);
        if(!juego)return res.status(404).json({error:"videojeugo no encontrado"})
        res.json({data:juego});
    }catch(error){next(error)}
